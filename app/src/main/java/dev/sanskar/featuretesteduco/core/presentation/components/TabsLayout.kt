    ...
    HorizontalPager(state = pagerState, count = 2) {page ->
        when (page) {
            ...
            1 -> LessonsScreen(
                ...
                resources = resources,
                imageLoader = imageLoader,
//                onNavigate = onLessonClick

//                onNavigate = onNavigate(Screen.LessonDetailScreen.route + "/${it._id}")}
...
fun LessonsScreen(
    ...
    LazyColumn(
        ...
        items(lessons) {
            println("fyulcvktyfvjdytftrjdfdtrjfdtrjfdujfdjtfhdjtf djhtfd trjyvd trjy")
            println(it._id)

            Lesson(lesson = it,
                imageLoader = imageLoader,
                onClick ={ onNavigate(Screen.LessonDetailScreen.route + "/${it._id}")})
            Spacer(modifier = Modifier.height(30.dp))
        }
