...
import dev.sanskar.featuretesteduco.core.presentation.components.FreeTrial
import dev.sanskar.featuretesteduco.core.presentation.components.MostWatchedCourses
import dev.sanskar.featuretesteduco.core.presentation.components.OthersWatchedCourses
import dev.sanskar.featuretesteduco.core.presentation.components.PopularCategory
import dev.sanskar.featuretesteduco.core.presentation.components.PreviousWatchedCourses
                ...
                Column(modifier = Modifier.fillMaxSize()) {
                    ...
                    LazyRow {
                        items(categoryPagingState.items.size) { i ->
                            ...
                            if (i >= categoryPagingState.items.size - 1 && !categoryPagingState.isLoading) {
                    ...
                    LazyRow {
                        items(mostWatchedCoursePagingState.items.size) { i ->
                            val course = mostWatchedCoursePagingState.items[i]
                            if (i >= mostWatchedCoursePagingState.items.size - 1 && !mostWatchedCoursePagingState.endReached && !mostWatchedCoursePagingState.isLoading) {
                                viewModel.loadNextPreviousWatchedCourses()
                            }
                    ...
                    LazyRow {
                        items(mostWatchedCoursePagingState.items.size) { i ->
                            val course = mostWatchedCoursePagingState.items[i]
                            if (i >= mostWatchedCoursePagingState.items.size - 1 && !mostWatchedCoursePagingState.endReached && !mostWatchedCoursePagingState.isLoading) {
                                viewModel.loadNextOthersWatchedCourses()
                            }
