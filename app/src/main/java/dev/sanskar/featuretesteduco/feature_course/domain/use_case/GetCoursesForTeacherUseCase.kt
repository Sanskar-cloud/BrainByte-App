...
class GetCoursesForTeacherUseCase @Inject constructor(
    private val coursesRepository: CourseRepository){
    suspend operator fun invoke(Teacherid: String, page: Int, pageSize: Int= Constants.DEFAULT_PAGE_SIZE): Resource<List<CourseOverview>> {
         return coursesRepository.getCoursesforTeacherPaged(
            Teacherid = Teacherid,
            page = page,
            pageSize = pageSize
        )
